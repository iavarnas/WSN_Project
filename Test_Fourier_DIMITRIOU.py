...
while (1==1):
    if serialInst.in_waiting:
        ...
        if ((Current_time-Start_FFT_time)>FFT_time_window):
            ...
            Average_time_difference=(Current_time-Start_FFT_time)/float(Time_accel.size - 1) # this is the constant time diff for FFT
            Fs = 1000/Average_time_difference # the maximum frequency
            Df=Fs/float(N_FFT) # the frequency step in the FFT
            Time_Sequence_For_FFT=np.linspace(Start_FFT_time,Current_time,N_FFT) # my time sequence is regularly spaced
            f = np.linspace(0, (N_FFT - 1) * Df, int(N_FFT))  # frequency steps, x axis in frequency domain

            X_accel_interp = np.interp(Time_Sequence_For_FFT, Time_accel, X_accel) # I am creating the interpolated values at the corresponding FFT time slots
